 ...
 */
/*
function gtw_preprocess_user_profile_form(&$variables) {
  $variables['field_name'] = drupal_render($variables['']['field_name']);
  $variables['field_username'] = drupal_render($variables['']['account']['name']);
  $variables['actions'] = drupal_render_children($variables['']); 
}
*/
function gtw_fboauth_action__connect($variables) {
  $action = $variables['action'];
  $link = $variables['properties'];
  $link['query']['scope'] = 'email,publish_actions';
...
function gtw_theme() {
  ...
  $items['block'] = array(
    ...
    'template' => 'object',
    'path' => drupal_get_path('theme', 'gtw') .'/templates',
  );
  $items['comment'] = array(
 ...
 */
function gtw_preprocess_html(&$vars) {
  $vars['classes_array'][] = 'gtw';
}

/**
 * Implementation of preprocess_page().
 */
function gtw_preprocess_page(&$vars) {
 ...
 */
function gtw_preprocess_block(&$vars) {
  $vars['hook'] = 'block';
 ...
 */
function gtw_preprocess_node(&$vars) {
  $vars['hook'] = 'node';
 ...
 */
function gtw_preprocess_comment(&$vars) {
  $vars['hook'] = 'comment';
 ...
 */
function gtw_preprocess_fieldset(&$vars) {
 ...
 */
function gtw_preprocess_field(&$vars) {
 ...
 */
function gtw_textarea($variables) {
 ...
 */
function gtw_views_mini_pager($vars) {
  $tags = $vars['tags'];
  $quantity = $vars['quantity'];
  $element = $vars['element'];
  $parameters = $vars['parameters'];
 ...
 */
function gtw_lt_username_title($variables) {
 ...
 */
function gtw_lt_login_link($variables) {
 ...
 */
function gtw_status_messages($variables) {
  ...
  foreach (drupal_get_messages($display) as $type => $messages) {
    ...
    $output .= "<div class=\"messages alert $status_class[$type]\">\n";
    $output .= '<button data-digtwiss="alert" class="close">×</button>';
 ...
 */
function gtw_field__taxonomy_term_reference($variables) {
  $output = '';

  // Render the label, if it's not hidden.
  //if (!$variables['label_hidden']) {
  //  $output .= '<icon class="icon-tag"></i> ';
  //}

  // Render the items.
  $output .= '<ul class="links item-labels">';
  foreach ($variables['items'] as $delta => $item) {
    $output .= '<li class="label item-label taxonomy-term-reference-' . $delta . '"' . $variables['item_attributes'][$delta] . '>' . drupal_render($item) . '</li>';
  }
  $output .= '</ul>';
